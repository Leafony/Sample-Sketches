...
void setupPort()
{
  //  pinMode(D2, INPUT); // [D2]  PA10 : not used
  pinMode(D3, INPUT); // [D3]  PB3  : not used
  pinMode(D4, INPUT); // [D4]  PB5  : not used
  pinMode(D5, INPUT); // [D5]  PB4  : not used
  pinMode(D6, INPUT); // [D6]  PB10 : not used

  pinMode(BLE_WAKEUP, OUTPUT);    // [D7]  PA8 : BLE Wakeup/Sleep
  digitalWrite(BLE_WAKEUP, HIGH); // BLE Wakeup

  //  pinMode(D8, INPUT); // [D8]  PA9  : not used
  //  pinMode(D9, INPUT); // [D9]  PC7  : not used
  pinMode(D10, INPUT); // [D10] PB6  : not used
  pinMode(D11, INPUT); // [D11] PA7  : not used
  pinMode(D12, INPUT); // [D12] PA6  : not used
  pinMode(D13, INPUT); // [D13] PA5  : not used
  pinMode(A2, INPUT);  // [A2]  PA4  : not used
  pinMode(A3, INPUT);  // [A3]  PB0  : not used
}
